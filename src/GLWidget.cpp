...
void GLWidget::reset()
{
	...
	m_create_type = Level::OBJECT_TYPE_COLLISION;
...
void GLWidget::selectPrefab(int object_id)
{
	m_prefab_object = object_id;
}

void GLWidget::setPrefabScale(double scale)
{
	m_prefab_scale = scale;
}

void GLWidget::setPrefabDepth(int depth)
{
	m_prefab_depth = depth;
}

void GLWidget::setPrefabAlign(int align)
{
	m_prefab_align = align;
}
...
void GLWidget::setTilemapConfig(int xstart, int xend, int ystart, int yend, float tile_size)
{
...
void GLWidget::setTiledPrefabSize(int width, int height)
{
...
void GLWidget::tilemapDraw(glm::vec2 mouse_lp)
{
	...
	{
		float fx = mouse_lp.x / m_tilemap_size;
		float fy = mouse_lp.y / m_tilemap_size;
	...
	switch (m_opmode)
	{
			...
			if (event->button() & Qt::LeftButton)
			{
				if (m_dragging)
				{
					...
					if (delta.x != 0.0f && delta.y != 0.0f)
					{
						...
						{
							case Level::OBJECT_TYPE_COLLISION:		name = "Collision"; break;
							case Level::OBJECT_TYPE_GEOVIS:			name = "Geometry"; break;
							case Level::OBJECT_TYPE_SLIDER:			name = "Slider"; break;
		...
		case MODE_PREFAB:
		{
			if (m_prefab_scaling)
			{
				...
				glm::vec2 delta = mouse_lp - m_prefab_scale_reference;
				m_prefab_scale = m_prefab_scale + delta.x;
				if (m_prefab_scale < 0.5f)
					m_prefab_scale = 0.5f;

				emit onPrefabScaleChange(m_prefab_scale);
			}
			break;
		}
...
void GLWidget::mousePressEvent(QMouseEvent* event)
{
	...
	switch (m_opmode)
	{
		...
		case MODE_PREFAB:
		{
			...
			{
				m_prefab_scaling = true;
				m_prefab_scale_reference = toLevelCoords(glm::vec2(mouse_x, mouse_y));
				if (m_snap_grid)
					m_prefab_scale_reference = snapToGrid(m_prefab_scale_reference);
			}
...
void GLWidget::keyReleaseEvent(QKeyEvent* event)
{
	...
	switch (m_opmode)
	{
		...
		case MODE_PREFAB:
		{
			if (key == Qt::Key_Space)
			{
				if (m_prefab_object >= 0)
				{
					glm::vec2 mouse_lp = toLevelCoords(glm::vec2(m_prefab_point.x(), m_prefab_point.y()));
					if (m_snap_grid)
						mouse_lp = snapToGrid(mouse_lp);

					glm::vec2 points[8];
					glm::vec2 uvs[8];
					const Level::Prefab* prefab = m_level->getPrefabById(m_prefab_object);

					glm::vec2 origin;
					switch (m_prefab_align)
					{
						case 0: origin = glm::vec2(prefab->anchor_leftx, prefab->anchor_topy); break;
						case 1: origin = glm::vec2(prefab->anchor_leftx, prefab->anchor_bottomy); break;
						case 2: origin = glm::vec2(prefab->anchor_rightx, prefab->anchor_topy); break;
						case 3: origin = glm::vec2(prefab->anchor_rightx, prefab->anchor_bottomy); break;
					}

					for (int i=0; i < prefab->num_points; i++)
					{
						points[i] = (glm::vec2(prefab->points[i].x - origin.x,  prefab->points[i].y - origin.y) * m_prefab_scale) + mouse_lp;
...
void GLWidget::initializeGL()
{
	...
	m_level_coll_program = new QGLShaderProgram(this);
	m_level_coll_program->addShaderFromSourceCode(QGLShader::Vertex, level_vs_file);
	m_level_coll_program->addShaderFromSourceCode(QGLShader::Fragment, level_coll_fs_file);
	m_level_coll_program->link();

	error = m_level_coll_program->log();
	errors = error.toStdString();

	m_level_coll_shader.position	= m_level_coll_program->attributeLocation("a_position");
	m_level_coll_shader.tex_coord	= m_level_coll_program->attributeLocation("a_texcoord");
	m_level_coll_shader.location	= m_level_coll_program->uniformLocation("v_location");
	m_level_coll_shader.scale		= m_level_coll_program->uniformLocation("v_scale");
	m_level_coll_shader.vp_matrix	= m_level_coll_program->uniformLocation("m_vp_matrix");
	m_level_coll_shader.rot_matrix	= m_level_coll_program->uniformLocation("m_rot_matrix");
...
void GLWidget::renderPrefabMode(QPainter& painter)
{
	QPoint mouse_p = this->mapFromGlobal(QCursor::pos());
	...
	glm::vec2 center_point = mouse_lp;

	float scale = m_prefab_scale;
	if (m_prefab_scaling)
	{
		glm::vec2 delta = mouse_lp - m_prefab_scale_reference;
		scale += delta.x;
		...
		center_point = m_prefab_scale_reference;
	}
	...
	if (m_prefab_object >= 0)
	{
		...
		for (int i=0; i < prefab->num_points; i++)
		{	
			glm::vec2 p = toScreenCoords(center_point + (glm::vec2(prefab->points[i].x - origin.x, prefab->points[i].y - origin.y) * scale));
			ppoints[i].setX(p.x);
			ppoints[i].setY(p.y);
		}
	...
	glm::vec2 ex = toScreenCoords(mouse_lp + glm::vec2(1.0f, 1.0f));

	painter.setPen(QColor(224,224,224));
	painter.setBrush(QBrush(QColor(224,224,224,128)));
	painter.drawRect(sx.x, sx.y, ex.x-sx.x, ex.y-sx.y);
	*/
}

void GLWidget::renderTiledprefabMode(QPainter& painter)
{
	QPoint mouse_p = this->mapFromGlobal(QCursor::pos());
...
void GLWidget::renderOtherObjects(QPainter& painter)
{
	...
	for (int i=0; i < num_objs; i++)
	{
			...
			render)
		{
			...
			else if (type == Level::OBJECT_TYPE_SLIDER ||
					 type == Level::OBJECT_TYPE_MOVER)
			{
				int num_points = obj->getNumPoints();
				if (num_points >= 2)
				{
...
{
	float xs = (float)m_tilemap_xstart * m_tilemap_size;
	float xe = (float)m_tilemap_xend * m_tilemap_size;
	float ys = (float)m_tilemap_ystart * m_tilemap_size;
	float ye = (float)m_tilemap_yend * m_tilemap_size;
...
//	glDisable(GL_CULL_FACE);
	...
	// collision objects
	m_level_coll_program->bind();

	m_level_coll_program->setUniformValue(m_level_coll_shader.vp_matrix, vp_mat);
	m_level_coll_program->setUniformValue(m_level_coll_shader.rot_matrix, rot_mat);
	m_level_coll_program->setUniformValue(m_level_coll_shader.location, m_scroll.x, m_scroll.y);
	m_level_coll_program->setUniformValue(m_level_coll_shader.scale, 1.0f, 1.0f);

	{
		float* static_geo = m_level->getVBO(Level::VBO_COLLISION);
		int num_static_verts = m_level->numVBOVerts(Level::VBO_COLLISION);

		bool render = render = m_display_filter & (1 << (Level::OBJECT_TYPE_COLLISION-1)); 

		if (render && num_static_verts > 0)
		{
			m_level_coll_program->enableAttributeArray(m_level_coll_shader.position);
			m_level_coll_program->setAttributeArray(m_level_coll_shader.position, (GLfloat*)static_geo, 3, sizeof(Level::VBO));
			m_level_coll_program->enableAttributeArray(m_level_coll_shader.tex_coord);
			m_level_coll_program->setAttributeArray(m_level_coll_shader.tex_coord, (GLfloat*)static_geo+3, 2, sizeof(Level::VBO));

			glBindTexture(GL_TEXTURE_2D, m_base_tex);
			glDrawArrays(GL_TRIANGLES, 0, num_static_verts);

			m_level_coll_program->disableAttributeArray(m_level_coll_shader.position);
			m_level_coll_program->disableAttributeArray(m_level_coll_shader.tex_coord);
		}
	}
	...
	for (int vbo = 0; vbo < Level::NUM_VBOS; vbo++)
	{
		...
		switch (vbo)
		{
			case Level::VBO_GEOVIS:			render = m_display_filter & (1 << (Level::OBJECT_TYPE_GEOVIS-1)); break;
	...
	switch (m_opmode)
	{
		...
		case MODE_PREFAB:		renderPrefabMode(painter); break;
		case MODE_TILEDPREFAB:	renderTiledprefabMode(painter); break;
	}
	...
	switch (m_opmode)
	{
		...
		case MODE_PREFAB:
		{
			modetext = tr("Prefab: X: %1, Y: %2").arg(mlp.x).arg(mlp.y);
			break;
		}
		case MODE_TILEDPREFAB:
		{
			modetext = tr("Tiled Prefab: X: %1, Y: %2").arg(mlp.x).arg(mlp.y);
			break;
		}
		case MODE_TILEMAP:
		{
		...
		case MODE_POLYPLUS:
		{
			modetext = tr("Collision Plus: X: %1, Y: %2").arg(mlp.x).arg(mlp.y);
			break;
		}
		case MODE_POLYMINUS:
		{
			modetext = tr("Collision Minus: X: %1, Y: %2").arg(mlp.x).arg(mlp.y);
			break;
		}
	}	
