...
void ObjectDesignerWidget::insertObject(QString& name)
{
	if (m_poly_closed)
	{
		glm::vec2 points[8];
		...
		for (int i=0; i < num_points; i++)
		{
			float x = (points[i].x - minx) / side;
			float y = (points[i].y - miny) / side;
	
			float w = (maxx-minx) / side;
			float h = (maxy-miny) / side;

			x += (1.0f - w) / 2.0f;
			y += (1.0f - h) / 2.0f;

			npoints[i] = glm::vec4(x, y, points[i].x, points[i].y);
		}
...
ObjectDesigner::ObjectDesigner(QWidget* parent, Level* level, ObjectPrefabs* prefabs) : QDockWidget("Object Designer", parent, 0)
{
	m_level = level;
	m_prefabs = prefabs;

	m_widget = new ObjectDesignerWidget(parent, m_prefabs, m_level);
	...
	m_insert_button = new QPushButton(tr("Insert"), this);
	m_insert_button->setFocusPolicy(Qt::NoFocus);
	connect(m_insert_button, SIGNAL(clicked()), this, SLOT(insert()));
	...
	m_control_toolbar->addWidget(m_insert_button);
...
void ObjectDesigner::insert()
{
	bool ok;
	QString name = QInputDialog::getText(this, tr("Enter prefab name"), tr("Name:"), QLineEdit::Normal, "", &ok);

	if (ok && !name.isEmpty())
	{
		m_widget->insertObject(name);
	}
}
