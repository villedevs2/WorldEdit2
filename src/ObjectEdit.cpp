...
ObjectEdit::ObjectEdit(QWidget* parent, Level* level) : QDockWidget("Object Edit", parent, 0)
{
	...
	m_type_box->addItem("Collision", QVariant(Level::OBJECT_TYPE_COLLISION));
	m_type_box->addItem("Geometry", QVariant(Level::OBJECT_TYPE_GEOVIS));
	m_type_box->addItem("Trigger", QVariant(Level::OBJECT_TYPE_TRIGGER));
	m_type_box->addItem("Slider", QVariant(Level::OBJECT_TYPE_SLIDER));
	...
	// geo/deco widget
	// ------------------------------------------------------------------------
	m_geo_widget = new QWidget;
	QBoxLayout* geo_layout = new QBoxLayout(QBoxLayout::LeftToRight);
	m_geo_widget->setLayout(geo_layout);
	// ------------------------------------------------------------------------
	...
	// slider widget
	// ------------------------------------------------------------------------
	m_slider_widget = new QWidget;
	
	QLabel* slider_angle_label = new QLabel("Angle");
	slider_angle_label->setMaximumWidth(50);
	
	m_slider_angle = new QDoubleSpinBox();
	m_slider_angle->setRange(0.0, 360.0);
	m_slider_angle->setSingleStep(5.0);
	
	QGridLayout* slider_layout = new QGridLayout();
	slider_layout->addWidget(slider_angle_label,		0, 0, 1, 1);
	slider_layout->addWidget(m_slider_angle,			0, 1, 1, 1);

	m_slider_widget->setLayout(slider_layout);

	connect(m_slider_angle, SIGNAL(valueChanged(double)), this, SLOT(sliderAngleChanged(double)));
	// ------------------------------------------------------------------------
	...
	m_stacked_widget->addWidget(m_geo_widget);
	m_stacked_widget->addWidget(m_trigger_widget);
	m_stacked_widget->addWidget(m_slider_widget);
...
void ObjectEdit::setDefaultType(Level::ObjectType type)
{
	switch (type)
	{
		case Level::OBJECT_TYPE_COLLISION:		m_default_type = ITEM_COLLISION; break;
		case Level::OBJECT_TYPE_GEOVIS:			m_default_type = ITEM_GEOVIS; break;
		case Level::OBJECT_TYPE_TRIGGER:		m_default_type = ITEM_TRIGGER; break;
		case Level::OBJECT_TYPE_SLIDER:			m_default_type = ITEM_SLIDER; break;
		...
		default: m_default_type = ITEM_GEOVIS; break;
	}
}
...
void ObjectEdit::select(int object_id)
{
	...
	if (m_selected_object >= 0)
	{
		...
		switch (obj->getType())
		{
			...
			{
				m_type_box->setCurrentIndex(ITEM_COLLISION);
				m_stacked_widget->setCurrentWidget(m_geo_widget);
				break;
			}

			case Level::OBJECT_TYPE_GEOVIS:
			{
				m_type_box->setCurrentIndex(ITEM_GEOVIS);
				m_stacked_widget->setCurrentWidget(m_geo_widget);
				break;
			}
			...
			{
				m_type_box->setCurrentIndex(ITEM_SLIDER);
				m_stacked_widget->setCurrentWidget(m_slider_widget);

				Level::Object::Param angle = obj->getParam(0);

				m_slider_angle->setValue((double)(angle.f));
				break;
			}
...
void ObjectEdit::setType(int index)
{
	if (m_selected_object >= 0)
	{
		...
		switch (index)
		{
			case ITEM_COLLISION:		new_type = Level::OBJECT_TYPE_COLLISION; break;
			case ITEM_GEOVIS:			new_type = Level::OBJECT_TYPE_GEOVIS; break;
			case ITEM_TRIGGER:			new_type = Level::OBJECT_TYPE_TRIGGER; break;
			case ITEM_SLIDER:			new_type = Level::OBJECT_TYPE_SLIDER; break;
