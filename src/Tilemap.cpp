...
Tilemap::Tilemap()
{
	...
	m_config.ystart = 0;
	m_config.yend = 50;
	m_config.tile_size = 1.0f;

	m_map = new int[50 * 50];
	memset(m_map, -1, sizeof(int) * 50 * 50);

	resize(0, 0, 50, 50, 1.0f);
...
bool Tilemap::resize(int x1, int y1, int x2, int y2, float tile_size)
{
	int new_width = x2 - x1;
...
void Tilemap::tesselateTile(int x, int y)
{
	...
	float tx1 = (float)(x + m_x) * m_tile_size;
	float tx2 = tx1 + m_tile_size;
	float ty1 = (float)(y + m_y) * m_tile_size;
	float ty2 = ty1 + m_tile_size;
